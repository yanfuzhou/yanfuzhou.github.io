...
- From data side, we can further use [ST_Hexagon](https://postgis.net/docs/ST_Hexagon.html)/[ST_HexagonGrid](https://postgis.net/docs/ST_HexagonGrid.html) to hash node and edge geometries. The potential benefits of doing this is the heuristic estimation in A* algorithm will become Dijkstra, so we will get an accuracy gain, however performance wise we need to compare the PostGIS KNN search and H3 index speed to deside whether there is an improvement. If we decide to use hexagonal grid to do routing, then we also need to implement some hexagon algorithms that published at [here](https://www.redblobgames.com/grids/hexagons/).
